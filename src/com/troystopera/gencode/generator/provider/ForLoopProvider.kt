...
internal class ForLoopProvider(
    ...
    override fun generate(varProvider: VariableProvider, record: GenRecord): ProviderResult {
        ...
        return ProviderResult(loop, arrayOf(loop), newRecord)
    }

    private fun genDeclaration(varName: String, up: Boolean, record: GenRecord): Declaration {
    ...
    private fun genComparison(varName: String, up: Boolean, record: GenRecord): Comparison<IntVar> {
        val type = if (up) {
        ...
        return if (randHardBool() && record.hasVarType(VarType.INT_PRIMITIVE)) {
            Comparison(type, Variable.of<IntVar>(varName), Variable.of<IntVar>(record.getRandVar(VarType.INT_PRIMITIVE)))
        } else {
